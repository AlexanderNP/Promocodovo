const categoryRows = document.querySelectorAll('.category__row');
const categoryItem = document.querySelector('.category-seven')
const categoryAdd = document.querySelector('.category__tab-add')

categoryAdd.addEventListener('click', () => {
  categoryItem.classList.remove('category-seven')
  categoryRows.forEach((item) => item.classList.remove('hidden'))
  categoryAdd.classList.add('hidden')
})


function showCode(el) {
  const promoCode = 'Cofreak'; // Ваш промокод
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  showLessButton = document.querySelector('.brands__btn-less')
